...
APP_SUBTITLE_PERIOD = "Período 2020 - 2024" # Añadido

# Paleta de colores personalizada (tonos azules, verdes, beiges/marrones claros)
# Puedes ajustar estos colores hexadecimales según tu preferencia.
...
def load_confrarias_from_excel(excel_file_path, sheet_name):
    ...
    if 'Ano' in df.columns and not df['Ano'].isnull().all():
        ...
        df = df[df['Ano'].between(2020, current_year, inclusive='both')] # Ajustado para año actual
    ...
    # Limpieza de nombres de especies (mantener datos crudos aquí, formatear para display más tarde)
    for col_text in ['COFRADIA', 'ESPECIE']:
        if col_text in df.columns and (df[col_text].dtype == 'object' or pd.api.types.is_string_dtype(df[col_text])):
            ...
            if col_text == 'ESPECIE':
                 ...
                 # No aplicar <i> aquí para mantener los datos limpios para filtros y agrupaciones
...
def load_empresas_data_nova_estrutura(file_path):
    ...
    if 'Ano' in df_e.columns and not df_e['Ano'].isnull().all():
        current_year = datetime.date.today().year
        df_e=df_e[df_e['Ano'].between(2020, current_year, inclusive='both')] # Ajustado
...
# Obtener todas las especies únicas para el dropdown
...
app.layout = dbc.Container([
    ...
    dbc.Row(dbc.Col(html.P(f"Análise interactivo de datos de Confrarías e Empresas extractoras de algas. {APP_SUBTITLE_PERIOD}", className="text-center text-muted mb-4"), width=12)), # Subtítulo modificado
                ...
                dbc.Col(dcc.Dropdown(
                    id='especie-dropdown', placeholder="Seleccionar Especie(s)", multi=True, value=['all'],
                     options=([{'label': 'Tódalas especies', 'value': 'all'}] +
                              [{'label': html.I(es), 'value': es} for es in todas_especies_unicas] # MODIFICADO: Especies en cursiva
                             )
                ), md=3, className="mb-2"),
                dbc.Col(dcc.Dropdown(
...
def filter_dataframe_generic(df_original, year_filter, entidades_seleccionadas_raw, nome_col_entidade_no_df, valor_para_todas_as_entidades_do_tipo, especies_filtro_raw, trimestre_filtro):
    ...
    if 'ESPECIE' in df.columns and especies_filtro and 'all' not in especies_filtro:
        # El filtro de especies usa los nombres crudos (sin <i>)
...
def update_importe_tempo_confrarias(year, entidades, especies, trimestre):
    ...
    fig = go.Figure(data=[go.Scatter(x=ts_df['data'], y=ts_df['Importe'],mode='lines+markers',name='Importe Confrarías (€)', line_shape='spline', fill='tozeroy', line_color=CUSTOM_COLOR_PALETTE[3])]) # Verde
...
def update_lonja_kg_tempo(year, entidades, especies, trimestre):
    ...
    if proc_c and not df_confrarias_cleaned.empty and all(c in df_confrarias_cleaned.columns for c in ['data', 'Lonja Kg']):
        ...
        if not filt_c.empty and pd.api.types.is_datetime64_any_dtype(filt_c['data']) and filt_c['Lonja Kg'].sum() > 0:
            ...
            if not ts_c.empty:
                fig.add_trace(go.Scatter(x=ts_c['data'],y=ts_c['Lonja Kg'],mode='lines+markers',name='Kg Confrarías',line=dict(color=CUSTOM_COLOR_PALETTE[0]), line_shape='spline', fill='tozeroy')) # Azul
                data_plotted = True
    ...
    if proc_e and not df_empresas_cleaned.empty and all(c in df_empresas_cleaned.columns for c in ['data', 'Lonja Kg']):
        ...
        if not filt_e.empty and pd.api.types.is_datetime64_any_dtype(filt_e['data']) and filt_e['Lonja Kg'].sum() > 0:
            ...
            if not ts_e.empty:
                fig.add_trace(go.Scatter(x=ts_e['data'],y=ts_e['Lonja Kg'],mode='lines+markers',name='Kg Empresas',line=dict(color=CUSTOM_COLOR_PALETTE[4]), line_shape='spline', fill='tozeroy')) # Verde
                data_plotted = True
    ...
    if proc_c and not df_confrarias_cleaned.empty and all(c in df_confrarias_cleaned.columns for c in ['data', 'CPUE']):
        ...
        if not filt_c.empty and not filt_c['CPUE'].isnull().all() and pd.api.types.is_datetime64_any_dtype(filt_c['data']):
            ...
            if not ts_c.empty and ts_c['CPUE'].sum() > 0 :
                 fig.add_trace(go.Scatter(x=ts_c['data'],y=ts_c['CPUE'],mode='lines+markers',name='CPUE Confrarías',line=dict(color=CUSTOM_COLOR_PALETTE[1]), line_shape='spline')) # Azul medio
                 data_plotted = True
    ...
    if proc_e and not df_empresas_cleaned.empty and all(c in df_empresas_cleaned.columns for c in ['data', 'CPUE']):
        ...
        if not filt_e.empty and not filt_e['CPUE'].isnull().all() and pd.api.types.is_datetime64_any_dtype(filt_e['data']):
            ...
            if not ts_e.empty and ts_e['CPUE'].sum() > 0:
                fig.add_trace(go.Scatter(x=ts_e['data'],y=ts_e['CPUE'],mode='lines+markers',name='CPUE Empresas',line=dict(color=CUSTOM_COLOR_PALETTE[5]), line_shape='spline')) # Verde claro
                data_plotted = True
...
def update_especies_lonja_kg_pie(year, entidades, especies_f, trimestre):
    ...
    # Formatear etiquetas de especies para cursiva
    ...
    fig.update_layout(template=PLOTLY_TEMPLATE,
                      ...
                      showlegend=False) # MODIFICACIÓN: Leyenda eliminada
    return fig
...
def update_prezo_distribucion_especie(year, entidades, especies, trimestre):
    ...
    filt_df_top_especies = filt_df[filt_df['ESPECIE'].isin(top_especies_raw)].copy() # Usar .copy() para evitar SettingWithCopyWarning
    if filt_df_top_especies.empty: return create_empty_figure()

    # Crear columna para display con cursiva
...
def update_esforzo_evolucion(year, entidades, especies, trimestre):
    ...
    proc_c, proc_e = determine_active_dfs(entidades, df_confrarias_cleaned, df_empresas_cleaned)
    data_found = False
    color_idx = 0 # Para ciclar por la paleta de colores
        ...
        if not filt_c.empty and pd.api.types.is_datetime64_any_dtype(filt_c['data']):
            if 'Nº PERSON' in filt_c.columns and filt_c['Nº PERSON'].sum(skipna=True) > 0:
                ...
                if not ts_c_person.empty:
                    fig.add_trace(go.Scatter(x=ts_c_person['data'],y=ts_c_person['Nº PERSON'],mode='lines+markers',name='Nº Persoas Confr.',line=dict(color=CUSTOM_COLOR_PALETTE[color_idx % len(CUSTOM_COLOR_PALETTE)]), line_shape='spline'), secondary_y=False)
                    color_idx += 1
                    data_found = True
            if 'DIAS TRABA' in filt_c.columns and filt_c['DIAS TRABA'].sum(skipna=True) > 0:
                ...
                if not ts_c_dias.empty:
                    fig.add_trace(go.Scatter(x=ts_c_dias['data'],y=ts_c_dias['DIAS TRABA'],mode='lines+markers',name='Días Trab. Confr.',line=dict(color=CUSTOM_COLOR_PALETTE[color_idx % len(CUSTOM_COLOR_PALETTE)]), line_shape='spline'), secondary_y=True)
                    color_idx += 1
                    data_found = True
        ...
        if not filt_e.empty and pd.api.types.is_datetime64_any_dtype(filt_e['data']):
            if 'Nº PERSON' in filt_e.columns and filt_e['Nº PERSON'].sum(skipna=True) > 0:
                ...
                if not ts_e_person.empty:
                    fig.add_trace(go.Scatter(x=ts_e_person['data'],y=ts_e_person['Nº PERSON'],mode='lines+markers',name='Nº Persoas Emp.',line=dict(color=CUSTOM_COLOR_PALETTE[color_idx % len(CUSTOM_COLOR_PALETTE)]), line_shape='spline'), secondary_y=False)
                    color_idx += 1
                    data_found = True
            if 'DIAS TRABA' in filt_e.columns and filt_e['DIAS TRABA'].sum(skipna=True) > 0:
                ...
                if not ts_e_dias.empty:
                    fig.add_trace(go.Scatter(x=ts_e_dias['data'],y=ts_e_dias['DIAS TRABA'],mode='lines+markers',name='Días Trab. Emp.',line=dict(color=CUSTOM_COLOR_PALETTE[color_idx % len(CUSTOM_COLOR_PALETTE)]), line_shape='spline'), secondary_y=True)
                    color_idx += 1
                    data_found = True
...
def update_kg_comparativa_anual(entidades, especies, trimestre):
    ...
    fig = px.bar(summary_df, x='Ano', y='Lonja Kg', color='Fonte', barmode='group',
                 ...
                 color_discrete_map={'Confrarías': CUSTOM_COLOR_PALETTE[0], 'Empresas': CUSTOM_COLOR_PALETTE[3]}) # Azul y Verde
    fig.update_traces(textposition='outside')
...
def update_kg_mes_ano_heatmap(entidades, especies, trimestre):
    ...
    try:
        ...
        fig = go.Figure(data=go.Heatmap(
            ...
            colorscale='Blues', hovertemplate='Ano: %{y}<br>Mes: %{x}<br>Kg: %{z:,.0f}<extra></extra>', # Paleta Blues para heatmap es común
...
def update_kg_mes_especie_heatmap(year, entidades, trimestre):
    df_list_hm_especie = []
    proc_c, proc_e = determine_active_dfs(entidades, df_confrarias_cleaned, df_empresas_cleaned)
    common_cols_esp = ['ESPECIE','MES_NOME','MES','Lonja Kg'] # Especie raw
    ...
    # Crear columna de display para el heatmap (eje Y)
    ...
    # Ordenar para que el heatmap mantenga un orden consistente si es posible
    ...
    try:
        ...
        fig = go.Figure(data=go.Heatmap(
            z=heatmap_pivot_esp.values, x=heatmap_pivot_esp.columns.astype(str), y=heatmap_pivot_esp.index, # y usará ESPECIE_display
            colorscale='Greens', hovertemplate='Especie: %{y}<br>Mes: %{x}<br>Kg: %{z:,.0f}<extra></extra>', # Paleta Greens
            colorbar=dict(title='Kg Totais')))
...
def update_prezos_evolucion_tempo(year, entidades, especies_filtro, trimestre):
    ...
    if not especies_a_considerar_para_plot_raw:
        ...
        if not ts_df_general.empty:
            fig.add_trace(go.Scatter(x=ts_df_general['data'], y=ts_df_general['Precio Kg en EUR'], mode='lines+markers', name='Prezo Medio Global (€/Kg)', line_shape='spline', line_color=CUSTOM_COLOR_PALETTE[6])) # Beige
            data_plotted = True
    else:
        ...
        if not df_para_plot.empty:
            ...
            if not ts_df_especies.empty:
                ...
                for trace_idx, trace in enumerate(fig_px.data):
                    # Asegurar que los colores se ciclan si hay más especies que colores en la paleta base
    ...
    fig.update_layout(template=PLOTLY_TEMPLATE, margin=dict(t=40, b=30, l=70, r=20),
                      legend=dict(title_text="<i>Especie</i>", orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1), # Leyenda título en cursiva
...
def update_kg_recollidos_especie_evolucion_anual_barras(year_filter, entidades, especies_filtro, trimestre):
    ...
    df_total['Ano'] = df_total['data'].dt.year.astype(str)
    ...
    top_especies_a_mostrar_raw = yearly_df.groupby('ESPECIE')['Lonja Kg'].sum().nlargest(7).index
    # Usamos ESPECIE_display para el color, así que necesitamos que esté en el df filtrado
    yearly_df_top = yearly_df[yearly_df['ESPECIE'].isin(top_especies_a_mostrar_raw)]
...
def update_cantidade_entidade_ano(year_filter, entidades_sel, especies, trimestre):
    ...
    fig = px.bar(summary_df_top, x='Entidade', y='Lonja Kg', color='Ano', barmode='group',
                 ...
                 text_auto='.2s', color_discrete_sequence=CUSTOM_COLOR_PALETTE)
    fig.update_traces(textposition='outside')
    fig.update_layout(template=PLOTLY_TEMPLATE, margin=dict(t=40, b=120, l=70, r=20),
...
def update_rentabilidade_especie(year, entidades, especies_filtro, trimestre):
    ...
    fig = px.bar(summary_df, y='ESPECIE_display', x='Rentabilidade_Persoa_Dia', orientation='h',
                 ...
                 color='Rentabilidade_Persoa_Dia', color_continuous_scale='Greens', text_auto='.2f') # Usar Greens
    fig.update_traces(textposition='outside')
...
def update_tabla_detallada_confrarias(year, entidades, especies, trimestre):
    ...
    # Formatear especie en cursiva para la tabla
    ...
    # Modificar la definición de columnas para permitir HTML en 'ESPECIE'
    disp_cols_f = []
    for i in final_cols:
        col_def = {"name": col_map.get(i, i), "id": i}
        if i == 'ESPECIE':
            col_def["presentation"] = "markdown" # o "html" if Dash version supports it broadly
    ...
    return dash_table.DataTable(
        ...
        markdown_options={'html': True} # Necesario para que `presentation: "markdown"` renderice HTML como <i>
    )
